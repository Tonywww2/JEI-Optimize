...
| Serial background startup | `async.asyncStartup` | true | Cross-version smoke and deterministic stop-cancellation pass. Known main-thread-only plugins are routed through `async.mainThreadPlugins`; runtime callbacks and publication always use the client thread. |
...
| A registration plugin requires the render thread | Medium | Plugin callbacks remain serial; known plugin IDs are selected by `async.mainThreadPlugins`, users can append a plugin or mod ID, and `asyncStartup=false` restores JEI's original caller-thread path. | Add newly confirmed plugins to the default list after reproducing their thread assertion. |
