...
- JEI Plugin 回调必须保持原顺序且不得并发。默认保持调用线程语义；CR-1 允许
  `asyncStartup` 将构建与注册序列移动到一条可取消的专用启动线程，最终
  `onRuntimeAvailable` 回调仍返回 client thread。
...
`asyncStartup` 是受控的前置例外：它不并行插件任务，而是将 JEI 构建与注册作为一个
single-flight 串行任务运行。最终 `onRuntimeAvailable` 回调和 runtime 发布返回 client
thread；退出世界会使 generation 失效并中断该任务。关闭此开关时恢复 JEI 原始调用线程。
