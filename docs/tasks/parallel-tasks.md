...
| CR-2 | 2026-09-17 | GitHub Copilot | Implement tooltip design Phase 0 with `diagnostics.tooltipSearchMetrics=false` and independent `TooltipSearchSnapshot`; no cache lifetime or worker safety relaxation. | User requested implementation of the design. Shadow diagnostics retain native search; suppression, persistent caching and a default-on worker search are not approved by this change. |
| CR-3 | 2026-09-17 | GitHub Copilot | Continue tooltip implementation through acceptance: `async.tooltipSearchIndex=false`, `syncOptimizations.tooltipStringCache=true`; client-tick extraction, ordinal-only worker suffix storage, GUI-registration completion gate and atomic client publication. | User requested continued implementation through acceptance. Cache is bounded and build-local (no cross-build UID reuse); live reload retains native synchronous rebuild. Public MethodHandles bridge the verified JEI internal package changes; unknown storage bypasses optimization. No default-on release or disk cache. |
| CR-4 | 2026-09-17 | GitHub Copilot | Implement measured tooltip batch pipeline: two waiting batches, byte backpressure, one writer with explicit stop acknowledgement; separate getter/add/build-step/client-tick/runtime timing; verify JEI15.59.0.212 and19.56.0.441 tooltip paths. | User requested implementation. Shared tick fairness, client-only callbacks, build-local cache and default-off flag remain unchanged. Allocation/recipe patches require new large-pack evidence; runtime publication is not a sidebar-interaction measurement. |
| CR-5 | 2026-09-17 | GitHub Copilot | Remediate measured ATM9 startup stalls: coalesce verified JEI grid layout callbacks during unpublished runtime delivery; route Blue Skies/Delightful to client; connect native budgeted builds to the existing GUI completion gate; correct cross-window frame metrics. | User requested plan and fixes. Only grid layout notifications are delayed; ingredient mutation, cache invalidation, third-party listeners and explicit queries stay synchronous. Flush before runtime publication; discard queued layouts on failure/stale generation. No callback parallelism or exception suppression. Existing async startup/filter switches retain control; third-party ABI/recipe failures remain separate acceptance blockers. |
| CR-6 | 2026-09-17 | GitHub Copilot | Separate MineColonies core caching from optional Tweaks ABI; make Iron's Spells compaction an atomic maker/recipe feature with exact descriptors and complete reflective dependencies. | User requested continued repairs. Existing defaults, thread routing and cache lifetimes unchanged. ATM9 MineColonies core JAR accepted without Tweaks; unsupported Iron's Spells3.4.0.11 stays native,3.16.3 Forge/NeoForge contracts accepted. No new performance claim or live instance deployment. |
| CR-7 | 2026-09-17 | GitHub Copilot | Add `syncOptimizations.fixMineColoniesAttributeModifiers=true` for verified Forge MineColonies JEI attribute calculations. Wrap the temporary instance's modifier consumer with vanilla equipment remove-then-add behavior. | User requested fixing MineColonies exceptions. Preserve original collection order, getter count, base value, operations, clamping and unrelated exception handling. Scope requires MineColonies JEI recipe registration; no player/citizen state changes or global attribute patch. A UUID pre-deduplication prototype failed mixed-operation numerical tests and was replaced, not shipped. |
| CR-8 | 2026-09-20 | GitHub Copilot | Guard pre-render bookmark layout against missing runtime; move coalesced layout flush after runtime installation. | User requested fixing the0.14.0/JEI15.59 inventory crash. Supersedes CR-5 flush ordering: callbacks remain serial, then runtime installation, then grid refresh on the same client task. Nullable runtime accessor is field-ABI gated across five JEI releases; no bookmark deletion, main-thread waits, or early partial-index publication. |
